...
// create new effect
  exports.createEffect=(effect)=>{
    return new Promise((resolve, reject)=>{
      const sql = 'INSERT INTO Effects(effect_id, effect_name, effect_description, effect_type) VALUES(?,?,?,?)'
      db.query(sql, [effect.effect_id, effect.effect_name, effect.effect_description, effect.effect_type], function(err){
        if(err){
          reject(err);
          return;
        }
